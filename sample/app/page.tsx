  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        api
          ? `/api/videoDetails?videoID=${videoID}&lang=${lang}`
          : `/api/subtitles?videoID=${videoID}&lang=${lang}&proxyURL=${encodeURIComponent(
              proxyURL
            )}`
      );
      if (!response.ok) {
      ...
      setSubtitles(result.subtitles ?? result.videoDetails.subtitles);
        ...
        <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 mb-6">
          ...
          <div className="flex gap-4 items-center p-2 rounded-md">
            <button
              onClick={fetchData}
              disabled={isFetching}
              className="flex-1 bg-indigo-600 text-white py-2 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 transition duration-150 ease-in-out disabled:opacity-50"
            >
              {isFetching
                ? "Fetching..."
                : `Fetch Data via "${api ? "videoDetails" : "subtitles"}"`}
            </button>
            <input
              id="api"
              type="checkbox"
              className="h-6 w-6 "
              onChange={(evt) => setApi(evt.target.checked)}
            />
            <label htmlFor="api">Switch API</label>
          </div>
        </div>
